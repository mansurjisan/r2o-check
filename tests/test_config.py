...
from r2o_check.config import Config, load_config
...
def test_load_with_variants(tmp_path: Path) -> None:
    (tmp_path / ".r2o-check.yml").write_text("variants:\n  coupled: true\n")
    c = load_config(tmp_path)
...
def test_invalid_standards_version_type(tmp_path: Path) -> None:
    (tmp_path / ".r2o-check.yml").write_text("standards_version: 11\n")
    with pytest.raises(ValueError, match="standards_version must be a string"):
        load_config(tmp_path)


def test_invalid_disabled_rules_type(tmp_path: Path) -> None:
    (tmp_path / ".r2o-check.yml").write_text("disabled_rules: not_a_list\n")
    with pytest.raises(ValueError, match="disabled_rules must be a list"):
        load_config(tmp_path)


def test_non_mapping_raises(tmp_path: Path) -> None:
    (tmp_path / ".r2o-check.yml").write_text("- item1\n- item2\n")
